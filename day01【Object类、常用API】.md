...
* `public String toString()`：返回该对象的字符串表示。即把对象的变成带有地址值的字符串
* `public boolean equals(Object obj)`：指示其他某个对象是否与此对象“相等”。即比较两个对象的地址值是否相等,相等返回true,否则返回false
...
` java.util.Date`类 表示特定的瞬间，精确到毫秒。日期时间
...
* `public long getTime()` 把日期对象转换成对应的时间毫秒值,精确到毫秒
...
* `public SimpleDateFormat(String pattern)`：用给定的模式和默认语言环境的日期格式符号构造SimpleDateFormat。//把日期格式化指定的字符串1988年04月10日,"yyyy年MM月dd日"
...
Calendar为抽象类，所以Calendar类在创建对象时并非直接创建，而是通过静态方法创建，返回子类对象，如下：
...
public class Demo06CalendarInit {
    public static void main(String[] args) {
        Calendar cal = Calendar.getInstance();//cal日历对象,里面年月日....等.var
    }    
}
```
...
原来StringBuilder/StringBuffer是个字符串的缓冲区，即它是一个容器，容器中可以装很多字符串。并且能够对其中的字符串进行各种操作,比如
...
* **装箱**：从基本类型转换为对应的包装类对象。//对象new出来开辟空间,看起来像个箱子,装箱
...
~~~java
Integer i = new Integer(666);//使用构造函数函数,装箱
...
~~~
## 5.3自动装箱与自动拆箱//记,基本数据类型和其对应的引用数据类型互相转换,系统自动完成,jdk1.5开始支持!!!
...
### 基本类型转换为String
   ...
   基本类型转换String方式有多种，这里只讲最简单的一种方式： 

~~~
基本类型直接与””相连接即可；如：34+""//"34"
~~~
...
> 注意:如果字符串参数的内容无法正确转换为对应的基本类型，则会抛出`java.lang.NumberFormatException`数字格式化异常。
